...
public class ContactRowMapper implements RowMapper<Contact> {
    ...
    public Contact mapRow(ResultSet rs, int rowNum) throws SQLException {
        ...
        contact.setCreatedAt(LocalDate.from(rs.getTimestamp("CREATED_AT").toLocalDateTime()));
        contact.setCreatedBy(rs.getString("CREATED_BY"));

        if(null!=rs.getTimestamp("UPDATED_AT")){
            contact.setUpdatedAt(LocalDate.from(rs.getTimestamp("UPDATED_AT").toLocalDateTime()));
        }
